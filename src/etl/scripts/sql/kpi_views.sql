...
CREATE UNIQUE INDEX IF NOT EXISTS idx_total_leave_days_per_employee_mv 
    ON dw.total_leave_days_per_employee_mv (empId, fiscal_start_date, fiscal_end_date);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_employee_details_mv 
    ON dw.employee_details_mv (empId, fiscal_start_date, fiscal_end_date);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_leave_trends_by_month_mv 
    ON dw.leave_trends_by_month_mv (year, month);

-- 4. Leave trend by fiscal year per leave type [trend : fiscal year]
...
-- 5. Leave trend by fiscal year per department and leave type
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_leave_request_distribution_by_department_and_leave_types_mv
    ON dw.leave_request_distribution_by_department_and_leave_types_mv (
        fiscal_start_date, 
        fiscal_end_date, 
...
-- ⚠️ Skipping unique index for top 10 since result is not guaranteed to be unique. Optional: create a serial ID column if needed.
